...
```{r, echo = F, out.width = "60%", fig.align="center", fig.cap = "The return really depends on what you were investing in. But so does the risk..."}
knitr::include_graphics("01-historical.jpg")
```
---
...
.large[
Assume 10 shares of stock "Get Rich Quick" is bought immediately after issuing a dividend and sold immediately after issuing a dividend one year later. The stock was bought for $22, sold for $27 and the dividend was $2. What was the holding period return?
]
...
\begin{equation}
HPR = \frac{P_1 - P_0 + D_1}{P_0} = \frac{27-22+2}{2}=31.8\%
\end{equation}

For 10 shares:

\begin{equation}
HPR = \frac{P_1 - P_0 + D_1}{P_0} = \frac{270-220+20}{20}=31.8\%
\end{equation}
...
knitr::include_graphics("03-assetclass.png")
```
...
```{r, echo = F, out.width = "60%", fig.align="center"}
knitr::include_graphics("05-normal.png")
```
...
```{r, echo = F}
table <- data.frame("Date" = rep(NA,6), "Price" = NA)
table[,1] <- c("Dec 31 2013", "Dec 31 2014", "Dec 31 2015", "Dec 31 2016", "Dec 31 2017", "Dec 31 2018")
table[,2] <- c(118.56, 150.15, 132.04, 162.98, 198.22, 204.18)
...
```{r, echo = F}
table$Annual_Return = NA
table$Return_Diff = NA
table$Return_Diff_Sq = NA
table[2:nrow(table),3] <- table[2:nrow(table),2]/table[1:(nrow(table)-1),2]-1
Ravg <- mean(table[,3],na.rm=T)
table[-1,4] <- table[-1,3] - Ravg
table[-1,5] <- table[-1,4]^2
knitr::kable(table, caption = "Debt vs Equity", format = "markdown")
```
...
knitr::include_graphics("04-riskpremium.png")
```
.large[
...
```{r, echo = F}
table <- data.frame("Stock" = rep(NA,2), "Invested_Amount" = NA, "Expected_Annual_Return" = NA)
table[,1] <- c("A","B")
table[,2] <- c("$100","$300")
table[,3] <- c("10%","20%")
knitr::kable(table, caption = "Debt vs Equity", format = "markdown")
...
.pull-left[
```{r, echo = F, out.width = "80%", fig.align="center", fig.cap = "Correlation = -1"}
knitr::include_graphics("06-corr-1.jpg")
```

```{r, echo = F, out.width = "80%", fig.align="center", fig.cap = "Correlation = +1"}
knitr::include_graphics("07-corr+1.png")
```
]
.pull-right[
```{r, echo = F, out.width = "80%", fig.align="center", fig.cap = "Correlation = 0.35"}
knitr::include_graphics("08-corr.jpg")
```
]
...
```{r, echo = F}
table <- data.frame("State" = rep(NA,3), "Probability"=NA, "Return_A" = NA, "Return_B" = NA)
table[,1] <- c("Recession", "Normal", "Boom")
table[,2] <- c("30%","40%","30%")
table[,3] <- c("0%","10%","20%")
table[,4] <- c("-5%","20%","35%")
knitr::kable(table, caption = "Debt vs Equity", format = "markdown")
...
knitr::include_graphics("09-div.png")
```
...
.pull-right[
```{r, echo = F, out.width = "100%", fig.align="center"}
knitr::include_graphics("10-div2.png")
```
]
...
.pull-left[
```{r, echo = F, out.width = "80%", fig.align="center"}
knitr::include_graphics("13-div5.png")
```
]
.pull-right[
```{r, echo = F, out.width = "100%", fig.align="center"}
knitr::include_graphics("12-div4.png")
```
]
...
```{r, echo = F, out.width = "60%", fig.align="center"}
knitr::include_graphics("14-div6.png")
```
...
.pull-left2[
```{r, echo = F, out.width = "100%", fig.align="center", fig.cap="Consider a world with many risky assets; we can still identify the opportunity set of risk-return combinations of various portfolios"}
knitr::include_graphics("20.png")
```
]
.pull-right2[
```{r, echo = F, out.width = "40%", fig.align="center", fig.cap="Father of Modern Portfolio Theory (and Nobel Laureate) Harry Markowitz"}
...
.pull-right[
```{r, echo = F, out.width = "100%", fig.align="center", fig.cap = "The section of the opportunity set above the minimum variance portfolio is the efficient frontier"}
knitr::include_graphics("15-div7.png")
```
]
...
```{r, echo = F, out.width = "60%", fig.align="center"}
knitr::include_graphics("16-japan.png")
```
